...
app.post('/api/contact', async (req, res) => {
  ...
  if (SMTP_HOST && SMTP_USER && SMTP_PASS) {
    try {
      ...
      // We don't fail the HTTP request just because SMTP failed, 
      // as the inquiry was successfully received and logged to console.
    }
  } else {
    console.log('ℹ SMTP is not configured. Message logged to console only.');
  }

  return res.status(200).json({ success: true, message: 'Message received and logged successfully.' });
});
